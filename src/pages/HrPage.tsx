...
function OverviewSection() {
  ...
  function handleSelectEmployee(id: string) {
    setSelectedId(id)
    if (!id) { setEmpAnalysis(null); return }
    const emp = employees.find((e) => e.id === id)
    if (!emp) { setEmpAnalysis(null); return }
    if (emp.username?.trim()) {
      const data = loadEmployeeAnalysis(emp.username.trim().toLowerCase())
      setEmpAnalysis(data)
    } else {
      setEmpAnalysis(null)
    }
  }
  const chatAnalysis = (selectedId && empAnalysis?.chatAnalysis) ? empAnalysis.chatAnalysis : null
  const faceResult = (selectedId && empAnalysis?.faceResult) ? empAnalysis.faceResult : null
  ...
  return (
    <>
      ...
      {selected && !selected.username?.trim() && (
        <div className="mb-5 rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm text-yellow-700">
          ⚠ No login username linked to this employee. Go to <strong>Employees → Edit</strong> and fill in the <strong>Login Username</strong> field to see their analysis.
        </div>
      )}
