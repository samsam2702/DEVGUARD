...
export function FerDashboardCard() {
  ...
  if (!faceResult) {
    return (
      <DashboardCard title="Facial Expression Analysis" description="Real-time FER signals via Hume AI" icon={ScanFace}>
        <EmptyState compact icon={ScanFace} title="Waiting for facial signals" description="FER analysis will appear here once the employee session begins." />
      </DashboardCard>
    )
  }
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between rounded-lg bg-(--color-surface-subtle) px-4 py-3">
          <div>
            ...
            <p className="text-lg font-semibold capitalize text-(--color-ink)">{faceResult.dominant}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-(--color-ink-faint)">Confidence</p>
            <p className="text-lg font-semibold text-(--color-ink)">{Math.round(faceResult.confidence * 100)}%</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-(--color-ink-faint)">Last updated</p>
            <p className="text-sm font-medium text-(--color-ink)">{faceResult.timestamp}</p>
          </div>
        </div>
        <div className="space-y-2">
          <p className="text-xs font-medium text-(--color-ink-faint)">Top emotions</p>
          {faceResult.emotions.map((e: any) => (
        ...
        {faceHistory.length > 1 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {faceHistory.slice(-6).reverse().map((r: any, i: number) => (
